...
> **TL;DR** — One-command ROS 2 Humble containerized dev environment based on `osrf/ros` desktop-full image (x86_64 only). Auto-detects UID/GID/GPU, supports X11 GUI forwarding, multi-stage build with smoke test verification.
>
> ```bash
> ./build.sh && ./run.sh
> ```
...
- ROS environment: `ROS_DISTRO`, `setup.bash` sourceable, `ros2` CLI, `rviz2` available
