...
> **TL;DR** — One-command ROS 2 Humble containerized dev environment based on `osrf/ros` desktop-full image (x86_64 only). Auto-detects UID/GID, supports X11 GUI forwarding, multi-stage build with smoke test verification.
>
> ```bash
> ./build.sh && ./run.sh
> ```
...
- **OSRF desktop-full**: full ROS 2 desktop environment including RViz2, Gazebo, etc.
...
> **Note**: This image uses `osrf/ros` which only supports **x86_64**. For ARM/Raspberry Pi, use [ros2_humble](https://github.com/ycpss91255-docker/ros2_humble) instead.
...
When the subtree sits at `my_robot_project/docker/osrf_ros2_humble/`:

- **IMAGE_NAME**: directory name is `osrf_ros2_humble` (not `docker_*`), so detection falls through to `.env.example` which has `IMAGE_NAME=osrf_ros2_humble` — works correctly.
...
| `IMAGE_NAME` | Image name | `osrf_ros2_humble` |
...
|:--------:|------|-------------|--------|
| 1 | Last directory matches `docker_*` → strip prefix | `/home/user/docker_osrf_ros2_humble` | `osrf_ros2_humble` |
| 2 | Scan path (right→left) for `*_ws` → use prefix | `/home/user/ros2_humble_ws/docker_osrf_ros2_humble` | `ros2_humble` |
...
```
/home/user/
├── docker_osrf_ros2_humble/ ← repo (current dir)
└── osrf_ros2_humble_ws/     ← detected as WS_PATH
```
...
| `sys` | `osrf/ros:humble-desktop-full-jammy` | OS base: user/group, locale, timezone |
...
Located in `smoke_test/ros_env.bats`, executed automatically during `docker build --target test` — **33 tests** total.
...
| Test | Description |
|------|-------------|
| `rviz2` | Available after sourcing ROS (desktop-full) |
| `rqt` | Available after sourcing ROS |
| `gazebo` | Available |
