...
## test/smoke_test/ros_env.bats (33)
...
| `ROS_DISTRO is humble` | Verify ROS_DISTRO equals "humble" |
| `ROS setup.bash exists` | Verify /opt/ros/humble/setup.bash exists |
| `ROS environment can be sourced` | Source setup.bash and confirm success |
| `ros2 CLI is available after sourcing ROS` | Verify ros2 command is on PATH |
| `ros2 --help works` | Verify ros2 --help executes successfully |

### GUI tools - desktop-full (3)

| 测试项目 | 说明 |
|----------|------|
| `rviz2 command is available after sourcing ROS` | Verify rviz2 is on PATH |
| `rqt command is available after sourcing ROS` | Verify rqt is on PATH |
| `gazebo command is available` | Verify gazebo/gz/ign is on PATH |

### ROS dev tools (2)

| 测试项目 | 说明 |
|----------|------|
| `colcon command is available` | Verify colcon build tool is on PATH |
| `colcon --help works` | Verify colcon --help executes successfully |
